...
void _Merge(int *arr, int begin1, int end1, int begin2, int end2, int *temp)
{
	...
	//拷贝回原来的空间
	memcpy(arr + _begin1, temp + _begin1, sizeof(int)*(end2 - _begin1 + 1));//特别注意，begin的值是会发生改变的
}
...
void CountSort(int *arr, int size)//计数排序
{
	...
	int *temp = (int *)malloc(sizeof(int)*(max - min + 1));//开辟统计数组空间
	memset(temp, 0, sizeof(int)*(max - min + 1));

	for (int i = 0; i < size; i++)//进行统计
	{
		temp[arr[i] - min]++;
	}

	int sub = 0;
	for (int i = 0; i < max - min + 1; i++)//数组回填
	{
		while (temp[i])
		{
			arr[sub++] = i + min;
